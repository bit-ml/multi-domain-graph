...
class Inference:
    ...
    def init_model(self):
        ...
        with torch.no_grad():
            ...
            self.model = sgdepth(
                ...
                opt.train_depth_grad_scale,
                opt.train_segmentation_grad_scale,
                # opt.train_domain_grad_scale,
                opt.train_weights_init,
                opt.model_depth_resolutions,
            ...
            if torch.cuda.is_available():
                ...
                )  # for inference model should be in eval mode and on gpu
            else:
    ...
    def preproc_image(self, image):
        ...
        self.input_image = to_tensor(
            image)  # save tensor image to self.input_image for saving later
    ...
    def inference(self, rgb_frame):
        ...
        # self.save_pred_to_disk(disps_pred, segs_pred) # saves results
        ...
        return disps_pred, segs_pred

    def save_pred_to_disk(self, depth_pred, segs_pred, output_path_img):
        ...
        depth_pred = depth_pred * (255 / depth_pred.max()
                                   )  # Normalize Depth to 255 = max depth
        depth_pred = np.clip(depth_pred, 0, 255)  # Clip to 255 for safety
        depth_pred = depth_pred.astype(
            np.uint8)  # Cast to uint8 for openCV to display

        depth_img = cv2.applyColorMap(
            depth_pred,
            cv2.COLORMAP_COOL)  # Use PLASMA Colormap like in the Paper
        ...
        depth_img = cv2.resize(depth_img,
                               (self.image_o_width, self.image_o_height))
