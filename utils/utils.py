...
class EnsembleFilter_TwdExpert_SSIM_Mixed_Normalized(torch.nn.Module):
    def __init__(self, threshold=0.5):
        super(EnsembleFilter_TwdExpert_SSIM_Mixed_Normalized, self).__init__()
        self.threshold = threshold
...
class EnsembleFilter_TwdExpert_SSIM_Mixed_Normalized_Th(torch.nn.Module):
    def __init__(self, threshold=0.5):
        super(EnsembleFilter_TwdExpert_SSIM_Mixed_Normalized_Th,
              self).__init__()
        self.threshold = threshold
...
class EnsembleFilter_TwdExpert_L1(torch.nn.Module):
    ...
    def twd_expert_distances(self, data):
        ...
        ssim_maps = []
        for i in range(n_tasks):
            ssim_map = torch.nn.functional.l1_loss(data[:, :, :, :, i],
                                                   data[:, :, :, :, -1],
                                                   reduction='none')
            ssim_maps.append(ssim_map)
...
class EnsembleFilter_Equal(torch.nn.Module):
    def __init__(self, threshold=0.5):
        super(EnsembleFilter_Equal, self).__init__()
        self.threshold = threshold
...
class EnsembleFilter_TwdExpert_MSSIM_Mixed_Normalized(torch.nn.Module):
    def __init__(self, threshold=0.5):
        super(EnsembleFilter_TwdExpert_MSSIM_Mixed_Normalized, self).__init__()
        self.threshold = threshold
    ...
    def twd_expert_distances(self, data):
        ...
        for i in range(n_tasks):
            ssim_map = get_ssim_score(data[:, :, :, :, -1],
                                      ...
                                      kernels[0],
                                      reduction=False)
            for j in np.arange(1, len(g_filters)):
                ssim_map = ssim_map + get_ssim_score(data[:, :, :, :, -1],
                                                     data[:, :, :, :, i],
                                                     g_filters[j],
                                                     n_chs,
                                                     kernels[j],
                                                     reduction=False)
            ssim_map = ssim_map / len(g_filters)
            ssim_maps.append(ssim_map)
