...
EXPERTS_PATH = r'/data/experts-output/'
...
if __name__ == "__main__":
    ...
    experts = os.listdir(EXPERTS_PATH)
    experts.sort()
    ...
    idx = 0
    videos = os.listdir(os.path.join(EXPERTS_PATH, experts[0], DATASET_PATH))
    videos.sort()
    videos = videos[0:50]
    for video_name in videos:
        img_path = os.path.join(EXPERTS_PATH, 'rgb', DATASET_PATH, video_name,
                                '%08d.npy' % 1)
        img = np.load(img_path)
        img = img_for_plot(img)

        for exp_name in experts:
            exp_path = os.path.join(EXPERTS_PATH, exp_name, DATASET_PATH,
                                    video_name, '%08d.npy' % 1)
